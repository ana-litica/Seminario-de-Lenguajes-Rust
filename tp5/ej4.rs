//El coverage del ejercicio original me dio 100%
...
impl EstadoPrestamo{
    fn mismo_estado(&self,otro_estado:EstadoPrestamo)->bool{
        match (self,otro_estado) {
            (EstadoPrestamo::Devuelto,EstadoPrestamo::Devuelto)=>true,
            (EstadoPrestamo::EnPrestamo,EstadoPrestamo::EnPrestamo)=>true,
            _=>false,
        }
    }
}
...
#[allow(unused)]
impl Prestamo {
    pub fn new(libro:Libro,cliente:Cliente,vencimiento:Fecha,estado:EstadoPrestamo)->Prestamo{
    ...
    fn mismo_prestamo(self,prestamo:&Prestamo)->bool{
        ...
        self.estado.mismo_estado(prestamo.estado.clone())
    }
...
impl Biblioteca{
    pub fn new(nombre:String,direccion:String)->Biblioteca{
        Self::inner_new(nombre, direccion, "src/tp5/archivo_biblioteca".to_string())
    }

    fn inner_new(nombre:String,direccion:String,path:String)->Biblioteca{
        Biblioteca { nombre, direccion, copias_libros: HashMap::new(), prestamos: Vec::new(), path_archivo:path }
    }
    ...
    fn escribir_en_archivo_json(&self)->Result<(),Errores>{
        ...
        let biblioteca_serializada=serde_json::to_string(&self).map_err(|_|Errores::ErrorDeSerializacionDeDatos)?;
        archivo.write_all(&biblioteca_serializada.as_bytes()).map_err(|_|Errores::ErrorDeEscrituraDeDatos)?;
        Ok(())
    }

    pub fn agregar_libro(&mut self, libro:&Libro,cantidad:u16){
        self.copias_libros.insert(libro.isbn.clone(), cantidad);
        if let Err(_) = self.escribir_en_archivo_json(){
            self.copias_libros.remove(&libro.isbn);
        } 
    }
    ...
    pub fn decrementar_disposicion(&mut self, libro:&Libro)->bool{
        ...
        if let Err(_)=self.escribir_en_archivo_json(){
            if let Some(libr)=self.copias_libros.get_mut(&libro.isbn){
                *libr+=1;
            }
            return false;
        }
    ...
    pub fn contar_prestamos_cliente(&self,cliente:Cliente)->u32{
        ...
        for prestamo in self.prestamos.clone(){
            if prestamo.cliente.mismo_cliente(&cliente) && prestamo.estado.mismo_estado(EstadoPrestamo::EnPrestamo){
                cantidad+=1;
            }
        }
        cantidad
    ...
    pub fn realizar_prestamo(&mut self, cliente:&Cliente,libro:&Libro,mut fecha:Fecha,cantidad_dias:u32)->bool{
        ...
        if self.contar_prestamos_cliente(cliente.clone())<5 && *cant.unwrap()>=1 {
            ...
            let nuevo_prestamo=Prestamo::new(libro.clone(),cliente.clone(),fecha,EstadoPrestamo::EnPrestamo);
    ...
    pub fn pretamos_vencidos(&self,fecha_actual:Fecha)->Vec<Prestamo>{
        ...
        for prest in self.prestamos.clone(){
            if fecha_actual.es_mayor(prest.clone().vencimiento) && prest.estado.mismo_estado(EstadoPrestamo::EnPrestamo){
                lista.push(prest);
            }
        }
        lista
    ...
    pub fn devolver_libro(&mut self,libro:&Libro,cliente:&Cliente,fecha_actual:Fecha)->bool{
        ...
        self.prestamos[posicion].estado=EstadoPrestamo::Devuelto;
        self.prestamos[posicion].devolucion=Some(fecha_actual);
        self.incrementar_disposicion(libro);
        
        if let Err(_)=self.escribir_en_archivo_json(){
            self.prestamos[posicion].estado=EstadoPrestamo::EnPrestamo;
            self.prestamos[posicion].devolucion=None;
            self.decrementar_disposicion(libro);
...
#[cfg(test)]
mod test{
    use crate::tp5::{fecha::Fecha,ej4::{Biblioteca,Libro,Genero,Prestamo,Cliente,EstadoPrestamo}};
    use std::fs;
    ...
    fn buscar_prestamo_test(){
       ...
       let mut biblio=Biblioteca::new("Biblioteca Central La Plata".to_string(),"Calle 50 N° 742, La Plata".to_string());
        ...
        let prestamo=Prestamo::new(libro2.clone(),cliente.clone(),Fecha::new(08,05,2026),EstadoPrestamo::EnPrestamo);
    ...
    #[test]
    fn agregar_libro_fallido_por_falla_en_creacion_archivo_test(){
        let path="src/tp5/testeo_de_errores_agregar_libro";
        let _ = std::fs::remove_file(path);
        let _ = std::fs::remove_dir(path);

        //El archivo existe pero es un directorio
        fs::create_dir(path).unwrap();

        let mut biblio=Biblioteca::inner_new("Biblioteca Central La Plata".to_string(),"Calle 50 N° 742, La Plata".to_string(),path.to_string());
        let libro1=Libro::new("9788497592208".to_string(),"Don Quijote de la Mancha".to_string(),"Miguel de Cervantes".to_string(),1376,Genero::Novela);
        
        assert!(biblio.copias_libros.len()==0);
        biblio.agregar_libro(&libro1, 2);
        assert!(biblio.copias_libros.len()==0);
    ...
    #[test]
    fn decrementar_disposicion_fallida_por_creacion_de_archivo_fallido_test(){
        let path="src/tp5/testeo_de_errores_decrementar_disposicion";
        let _ = std::fs::remove_file(path);
        let _ = std::fs::remove_dir(path);
        ...
        //inserto para que no falle en el agregar
        biblio.copias_libros.insert(libro1.isbn.clone(), 2);
        assert!(biblio.copias_libros.get_key_value(&libro1.isbn)==Some((&"9788497592208".to_string(),&2)));
        biblio.decrementar_disposicion(&libro1);
        assert!(biblio.copias_libros.get_key_value(&libro1.isbn)==Some((&"9788497592208".to_string(),&2)));
    ...
    #[test]
    fn incrementar_disposicion_fallida_por_creacion_de_archivo_fallido_test(){
        let path="src/tp5/testeo_de_errores_incrementar_disposicion";
        let _ = std::fs::remove_file(path);
        let _ = std::fs::remove_dir(path);
        ...
        biblio.copias_libros.insert(libro1.isbn.clone(), 2);
        assert!(biblio.copias_libros.get_key_value(&libro1.isbn)==Some((&"9788497592208".to_string(),&2)));
        biblio.incrementar_disposicion(&libro1);
        assert!(biblio.copias_libros.get_key_value(&libro1.isbn)==Some((&"9788497592208".to_string(),&2)));
    ...
    #[test]
    fn realizar_prestamo_fallido_por_creacion_de_archivo_fallido_test(){
        let path="src/tp5/testeo_de_errores_realizar_prestamo";
        let _ = std::fs::remove_file(path);
        let _ = std::fs::remove_dir(path);
    ...
    #[test]
    fn devolver_libro_fallido_por_creacion_de_archivo_fallido_test(){
        let path="src/tp5/testeo_de_errores_devolver_libro";
        let _ = std::fs::remove_file(path);
        let _ = std::fs::remove_dir(path);
        ...
        biblio.devolver_libro( &libro1,&cliente,Fecha::new(12,05,2026));
        assert!(biblio.prestamos.len()==0);
